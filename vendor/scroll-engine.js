...
function mountScrollWorld(container, config) {
  ...
  if (config.brand) {
    ...
    brand.setAttribute('aria-label', config.brand.name || 'Home');
  ...
  let worldInView = true, animationFrameId = 0, lastRafTime = 0;
  ...
  function loadClip(s) {
    ...
    if (reduce || container.classList.contains('user-reduced-motion') || container.classList.contains('skip-media-load') || s.loading || s.failed || !s.clip) return;
    s.loading = true;
  ...
  function read() {
    ...
    for (let i = 0; i < NSEG; i++) {
      const s = SEGMENTS[i];
      const untouchedLanding = y < 2 && i === 0;
      if (!untouchedLanding && y > s.start - 1.6 * vh && y < s.end + 1.6 * vh) loadClip(s);
      ...
      if (!s.hasClip || !s.ready) {
        const sc = (reduce || container.classList.contains('user-reduced-motion')) ? 1 : 1.03 + local * 0.14;
    ...
    for (let i = 0; i < N; i++) {
      ...
      if (c._interactive !== interactive) {
        c._interactive = interactive;
        c.classList.toggle('is-interactive', interactive);
        c.toggleAttribute('inert', !interactive);
        c.querySelectorAll('a, button, input, select, textarea, [tabindex]').forEach(node => {
          if (!interactive) {
            if (!node.hasAttribute('data-sw-tabindex')) node.setAttribute('data-sw-tabindex', node.getAttribute('tabindex') || '');
            node.setAttribute('tabindex', '-1');
          } else if (node.hasAttribute('data-sw-tabindex')) {
            const original = node.getAttribute('data-sw-tabindex');
            if (original) node.setAttribute('tabindex', original); else node.removeAttribute('tabindex');
            node.removeAttribute('data-sw-tabindex');
          }
        });
      }
  ...
  function scheduleRaf() {
    if (animationFrameId || reduce || !worldInView || document.hidden || container.classList.contains('user-reduced-motion')) return;
    animationFrameId = requestAnimationFrame(raf);
  }

  function raf(now) {
    animationFrameId = 0;
    if (reduce || !worldInView || document.hidden || container.classList.contains('user-reduced-motion')) return;
    const dt = Math.min(50, lastRafTime ? now - lastRafTime : 1000 / 60);
    lastRafTime = now;
    const alpha = 1 - Math.exp(-dt / (isMobile() ? 70 : 55));
    const eps = isMobile() ? 0.018 : 0.009;
    for (let i = 0; i < NSEG; i++) {
      const s = SEGMENTS[i];
  ...
  document.addEventListener('visibilitychange', scheduleRaf);
  container.addEventListener('scrollworld:motionchange', () => { read(); scheduleRaf(); });
  if ('IntersectionObserver' in window) {
    const worldObserver = new IntersectionObserver(entries => {
      worldInView = entries[0] ? entries[0].isIntersecting : true;
      if (worldInView) scheduleRaf();
    });
    worldObserver.observe(container);
  }
  layout();
  scheduleRaf();
...
function injectCSS() {
  ...
  .sw-copy__cta{display:flex;flex-wrap:wrap;gap:12px;margin-top:28px;pointer-events:none;}
  .sw-copy.is-interactive .sw-copy__cta{pointer-events:auto;}
